...
**0923 答疑后暂不可作为现行提交稿：** 多个章节仍预填旧“事故或未遂至少一条”标签、旧分层折、20＋40 代理回测和相应实测数字。编译出的 PDF 会在封面后显示失效提示；各章节的旧数字只作历史快照，不得作为新官方口径的模型成绩引用。全项目共同事实见 [ADR-0006](../../decisions/ADR-0006-official-qa-foundation.md)；各章由原负责人依据新版证据分别修订，本任务不替他们调整算法方法。
