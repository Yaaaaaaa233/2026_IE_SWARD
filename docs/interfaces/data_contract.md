...
来源：任务一数据线（[DATACON-001](../tasks/DATACON-001.json)）。本文只定义字段、类型、语义与规则层方法；全部示例值为**合成数据**，仅示意格式。含真实统计的详细分析报告保存于受控本地目录，按 [数据边界](../DATA_POLICY.md) 不入公开仓库。
...
| monthly_avg_mileage | float | km | 画像月均里程；统计期未确认前不入模型 |
...
| y | int | 1 = 窗口内出现任一指定事故/未遂编码；0 = 日志完整假设下无记录 |
...
3. 重复报警同时保留"原始条数 / 时间段合并数 / 出现天数"三种形态；段合并阈值只作敏感性试验，标签永远不用段合并。
