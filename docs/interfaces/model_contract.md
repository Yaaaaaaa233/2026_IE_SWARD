...
本文只定义模型线的输入、评估协议、输出与命名规则；字段示例全部为**合成数据**。上游数据语义以[数据契约 v0.1](data_contract.md) 为准，本文不重复定义。
...
| splits | 数据契约（冻结） | 全队共用车辆级分层五折；方案 A/B 同折分对照 |
...
| 项 | 默认值 | 说明 |
| --- | --- | --- |
| lookback_days | 20 | 特征窗长度；0918 会议的 40 天口径保留为快速对照参数 |
| horizon_days | 40 | 标签窗长度；与数据线落地口径一致 |
| split | 契约 splits 冻结版 | 车辆级分层五折；另支持 holdout_350_150 作为快速对照 |
...
| forecast_result.csv | 组委会（官方模板） | gpsno, accident, risk_prob 三列；500 行全覆盖、概率 [0,1]、无重复 |
