...
4. 实现 B0 等存在透明弱基线。它只使用通过暴露门的行为／情境编码，设备异常只影响置信度，证据不足返回保守先验 50，不替代 R1。名称由 T2R-006 从 Q0 调整为 B0，Q0 保留给 R1 学习规则分。
...
本任务没有使用真实数据，也没有生成真实 AUC、Lift、子群或人因结论。B0 是刻意简单的效果下界；R1 的 Q0 仍由娄澜负责。Q1 不进入公开总分；是否融合属于 R3。历史事故／未遂的一票降级上限已经作为可选接口实现，但 T2-R0 D2 未经组长裁决前默认关闭。
