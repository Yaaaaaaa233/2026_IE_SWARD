...
> 0923 赛题公共规则已由 [ADR-0006](../decisions/ADR-0006-official-qa-foundation.md) 校正；未遂计数的团队约定见 [ADR-0007](../decisions/ADR-0007-near-miss-count-unit.md)。下文 S1 的 20＋40 和旧折分是历史代理回测设计，需在新标签下另行确认。此处不决定算法线调整。

## 1. 项目一句话

基于 500 台货车及 6/1–7/31 共 61 天可用历史，交付两个任务：**任务一**预测 8/1 起未来 40 天的风险（`11803` 事故至少 1 次或 `11804` 未遂至少 2 次为正类，线上初评按 `risk_prob` AUC）与**任务二**可解释、可运营的司机安全评分体系。提交截止 2026-10-15，决赛 11 月下旬。
...
| S1 评估 harness | S0 完成 | 按 ADR-0006 明确新版标签、时间截点和折分后，固定同口径 AUC 与辅助指标；20＋40 仅为旧代理设计 | 私改折分或指标口径 |
...
里程碑对齐：交付倒排 M0（管道审计）→ M1（底座基线）→ M2（特征验证）→ M3（评分体系）→ M4（深化文档）→ M5（终检提交 10-12 至 10-15），与特征阶段 A–D 及实验编号 F/T/I/X/D、E-B 的映射见 [plans/model-route-proposal.md](model-route-proposal.md)。
