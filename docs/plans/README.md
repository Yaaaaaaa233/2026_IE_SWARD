...
- [FEAT-014 叶安线特征与模型持续迭代探索](feat-014-adaptive-feature-model-exploration.md)：E0–E3 技术执行和 25 版独立机器审计已完成，任务进入 `review`；没有候选通过既有 AUC 区间主门，不能认定稳定提升。真实指标、OOF 与图表仅在受控 `outputs/feat-014/`，图表人工复核待记录。
